...
<Steps>
  ...
  <Step title="Authentication check">
    Snippet checks whether the recipient is signed in. If they are not, they are redirected to the login page with the invite URL URL-encoded as a `redirect` parameter:

    ```text
    /login?redirect=%2Finvite_user%3Fgroup_id%3D<group-uuid>
    ```

    After signing in with Google, the auth callback redirects them back to the invite URL automatically.
  </Step>
